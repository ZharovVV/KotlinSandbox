 ...
 *
 * CoroutineContext - является набором параметров для выполнения корутины (на каком диспатчере, с каким
 * именем корутины, с каким типом джобы и т.п.) Т.е. через контекст мы конфигурируем нашу корутину.
 *
 ...
 * * также это работает и в обратном порядке в случае возникновения ошибки (CancellationException не считается ошибкой).
 ...
 * отменялась только дочерняя джоба, в которой произошла ошибка,
 * необходимо использовать специальную реализацию джоб - [SupervisorJob].
